...
namespace BitzBytez.codes.DAL
{
    public class CartDA
    {
        ...
        public static int updateCartShippingAddress(int cartID, int shippingAddressID)
        {
            String queryStr = "UPDATE Cart SET shippingAddress = @s WHERE cartID = @cid";

            SqlConnection conn = new SqlConnection(connStr);
            SqlCommand cmd = new SqlCommand(queryStr, conn);
            cmd.Parameters.AddWithValue("@cid", cartID);
            cmd.Parameters.AddWithValue("@s", shippingAddressID);
        ...
        public static int updateCartPaidDate(int cartID)
        {
            String queryStr = "UPDATE Cart SET datePaid = GETDATE() WHERE cartID = @cid";
        ...
        {
            String queryStr = "SELECT DISTINCT datePaid FROM Cart WHERE accountID = @aid AND status = 'paid'";
