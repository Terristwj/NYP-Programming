...
public partial class DoctorRoomView : System.Web.UI.Page
{
    static string command = "";
    static string rewriteCommand = "";
    protected void Page_Load(object sender, EventArgs e)
    {
        command = "SELECT *, ua.name bookedName FROM Booking b";
        command += " INNER JOIN Doctor d ON d.Doctor_ID = b.Doctor_ID";
        command += " INNER JOIN userAccounts ua ON ua.userID = b.userID";
        command += " INNER JOIN DoctorRoom dr ON dr.bookingID = b.bookingID";
    ...
    protected void gv_Products_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Join")
        {
            ...
            Response.Redirect("~/DoctorJoiningRoom.aspx" + queryString);
        }
    }
    ...
    {
        string queryString = "?login=true" + "&userID=" + Request.QueryString["userID"] + "&p=1";
