...
public partial class ViewMyBookings : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["revoke"] != null)
        {
            Response.Write("<script>alert('Booking Revoked.')</script>");
        }
    }
    ...
    protected void gv_Products_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Revoke")
        {
            ...
            int number = rnd.Next(1000000, 9999999); // creates a number between 1000000 and 9999999
            string password = number.ToString();

            //Save into DB
            SqlCommand cmd = new SqlCommand("Update Booking Set userID = null , status = 'Null', roomPassword=" + password + " Where bookingID='" + e.CommandArgument + "' ", conn);
            int updated = cmd.ExecuteNonQuery();

            conn.Close();

            //Refresh webpage
            string queryString = "?login=true" + "&userID=" + Request.QueryString["userID"] + "&revoke=true";
            Page.Response.Redirect("~/ViewMyBookings.aspx" + queryString, true);
        }
    }
