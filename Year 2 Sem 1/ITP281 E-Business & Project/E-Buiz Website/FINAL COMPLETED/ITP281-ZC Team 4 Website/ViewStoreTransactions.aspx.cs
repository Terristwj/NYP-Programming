...
public partial class ViewStoreTransactions : System.Web.UI.Page
{
    static string command = "";
    static string rewriteCommand = "";
    protected void Page_Load(object sender, EventArgs e)
    {
        command = "SELECT * FROM StoreOrders so";
        command += " LEFT JOIN userAccounts ua ON ua.userID = so.userID";
        command += " WHERE ua.userID IS NOT NULL";
        rewriteCommand = "SELECT * FROM StoreOrders WHERE userID IS NO NULL";
        rewriteCommand += " LEFT JOIN userAccounts ua ON ua.userID = so.userID";
        rewriteCommand += " WHERE ua.userID IS NOT NULL";
        if (!IsPostBack)
        {
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        ...
        {
            tb_SearchUserID.Text = "";
            condition_Requirement = "AND Order_ID = '" + value + "'";
        }
        ViewGridView("Search", condition_Requirement);
    }

    protected void btn_SearchUserID_Click(object sender, EventArgs e)
    {
        string value = tb_SearchUserID.Text;
        string condition_Requirement = "";
        if (value.Equals(""))
        {
        ...
        {
            tb_Search.Text = "";
            condition_Requirement = "AND so.userID = '" + value + "'";
        }
        ViewGridView("Search", condition_Requirement);
    }
